...
public static class MachineStateReducers
{
    ...
    public static ReductionResult<string, MachineState> StartWordDefinition(MachineState state, string _) =>
        state.PushScope(WordSyntacticScope.Initial);

    public static ReductionResult<string, MachineState> BeginWord(MachineState state, string word) =>
        state.PopScope(out SyntacticScope scope).PushScope(((WordSyntacticScope)scope).SetWord(word));
    ...
    public static Reduce<string, MachineState> AddInnerWord(StateId gotoStateId)
    {
        ...
        {
            var nextState = state.PopScope(out SyntacticScope scope)
                .PushScope(((WordSyntacticScope)scope).AddInnerWord(innerWord));
    ...
    {
        var state2 = state.PopScope(out SyntacticScope scope);
        if (scope is not WordSyntacticScope wordScope)
